...
$\begin{align*} r_t &= \mu + \phi r_{t - 1} + \epsilon_t \\ \epsilon_t &=
\sigma_t \nu_t \\ \sigma_t^2 &= \omega + \alpha \epsilon_{t - 1}^2 + \beta
\sigma_{t - 1}^2 \end{align*}$
...
$\begin{align*} \{\hat{r}_{T + 1}\} &= \mu + \phi r_T + \{\hat{\epsilon}_{T +
1}\} \\ \{\hat{\epsilon}_{T + 1}\} &= \{\hat{\nu}_{T + 1}\} \times
\hat{\sigma}_{T + 1} \\ \hat{\sigma}_{T + 1}^2 &= \omega + \alpha
\epsilon_{T}^2 + \beta \sigma_{T}^2 \end{align*}$
...
$\begin{align*} \{\hat{r}_{T + 2}\} &= \mu + \phi \{r_{T + 1}\} +
\{\hat{\epsilon}_{T + 2}\} \\ \{\hat{\epsilon}_{T + 2}\} &= \{\hat{\nu}_{T +
2}\} \times \{\hat{\sigma}_{T + 2}\} \\ \{\hat{\sigma}_{T + 2}^{2}\} &= \omega +
\alpha \{\epsilon_{T + 1}^2\} + \beta \sigma_{T + 1}^2 \end{align*}$
...
$\{\hat{\nu}_{T + 1}\}$). Once $\{\hat{r}_{T + 1}\}$ and $ \{\hat{r}_{T + 2}\}$
are generated, VaR at $T + 2$ at any percentile, say, 5% can be picked off as
the 5% percentile of $\{\hat{r}_{T + 2}\}$.
