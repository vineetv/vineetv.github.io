...
number $latex \lambda$ between 0 and 1, perhaps close to 1, and multiply
historical observations with powers of. So, yesterday’s return could be
multiplied by $latex \lambda$ and the one before by $latex \lambda^2$ and so
on. That was the basic idea and so now you could use all historical data, and if
something happened long ago it wouldn’t matter much anymore, unless the things
that happened were like really major. One needs to do a bit more work to
identify the VaR, but the idea is intuitive.
...
If $latex r_t$ is the series of historical returns, Hull-White suggested
applying HS to volatility weighted returns series as: $latex r_t = r_t \times
\hat{\sigma}_T/\hat{\sigma}_t $, where $latex \hat{\sigma}_t$ is the estimated
volatility at time $latex t$ from, say, a GARCH model and $latex \hat{\sigma}_T$
...
model has already been fitted to the returns series $latex \{r_t\}$.

$latex \begin{aligned} r_t &= \mu + \phi r_{t - 1} + \epsilon_t \\ \epsilon_t &=
\sigma_t \nu_t \\ \sigma_t^2 &= \omega + \alpha \epsilon_{t - 1}^2 + \beta
\sigma_{t - 1}^2 \end{aligned}$

The distribution for $latex r_{T + 1}$ is generated by applying bootstrap on
estimated standardized residuals $latex \hat{\nu}_t =
\hat{\epsilon}_t/\hat{\sigma}_t$. Denoting the first such bootstrapped sample by
$latex \{\hat{\nu}_{T + 1}\}$, the next step is multiplying bootstrapped $latex
\{\hat{\nu}_{T + 1}\}$ with the forecast of $latex \hat{\sigma}_{T + 1}$, as:

$latex \begin{aligned} \{\hat{r}_{T + 1}\} &= \mu + \phi r_T +
\{\hat{\epsilon}_{T + 1}\} \\ \{\hat{\epsilon}_{T + 1}\} &= \{\hat{\nu}_{T +
1}\} \times \hat{\sigma}_{T + 1} \\ \hat{\sigma}_{T + 1}^2 &= \omega + \alpha
\epsilon_{T}^2 + \beta \sigma_{T}^2 \end{aligned}$

The algorithm for generating a distribution for $latex r_{T + 2}$ and beyond
proceeds similarly, except that now there are as many possible values for $latex
\{\hat{\sigma}_{T + 2}\}$ as bootstrapped $latex \{\hat{\epsilon}_{T + 1}\}$.

$latex \begin{aligned} \{\hat{r}_{T + 2}\} &= \mu + \phi \{r_{T + 1}\} +
...
In terms of multiplication of arrays, the product $latex \{\hat{\nu}_{T + 2}\}
\times \{\hat{\sigma}_{T + 2}\}$ is to interpreted in the sense of element-wise
multiplication ($latex \{\hat{\nu}_{T + 2}\}$ represents a separate bootstrap
from $latex \{\hat{\nu}_{T + 1}\}$). Once $latex \{\hat{r}_{T + 1}\}$ and $latex
\{\hat{r}_{T + 2}\}$ are generated, VaR at $latex T + 2$ at any percentile, say,
5% can be picked off as the 5% percentile of $latex \{\hat{r}_{T + 2}\}$.
